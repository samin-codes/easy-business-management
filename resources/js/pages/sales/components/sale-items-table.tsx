...
import type { Product, SaleItemFormData } from '../types';
            ...
            <table className="table-hover table min-w-240">
                ...
                <tbody>
                    {items.map((item, index) => {
                        const variant = variants.find((value) => value.id.toString() === item.product_variant_id);
                            ...
                            <tr key={item.uid}>
                                ...
                                <td className="w-32">
                                    <Input
                                        type="number"
                                        value={item.quantity}
                                        onChange={(event) => onItemChange(item.uid, { quantity: event.target.value })}
                                        onBlur={() => onItemChange(item.uid, { quantity: formatInteger(item.quantity) })}
                                ...
                                <td className="w-32 px-3 py-2">
                                    <Input
                                        ...
                                        onChange={(event) => onItemChange(item.uid, { unit_price: event.target.value })}
                                        onBlur={() => onItemChange(item.uid, { unit_price: formatDecimal(item.unit_price) })}
                                        className="no-number-spinner text-right"
                                        disabled={!variant}
